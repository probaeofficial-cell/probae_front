...
interface AsyncPlanTierSelectProps {
  value: string | "";
  onChange: (ulid: string) => void;
  selectedPlan?: any;
}

export default function AsyncPlanTierSelect({ value, onChange, selectedPlan }: AsyncPlanTierSelectProps) {
  ...
  const fetchOptions = async (pageNum: number, searchTerm: string, isNewSearch: boolean) => {
    setIsLoading(true);
    try {
      const res = await endpoints.planTiers.list({ page: pageNum, limit: 10, search: searchTerm }) as any;
      const newItems = res.tiers || res.data || [];
      if (isNewSearch) {
        setOptions(newItems);
      } else {
  ...
  const currentOption = options.find(o => (o.ulid || o._id) === value) || selectedPlan;
  const displayLabel = value === "" ? "Select Plan Tier..." : currentOption ? currentOption.name : "Select Plan Tier...";
        ...
        <div className="absolute z-[300] w-full mt-2 bg-white rounded-2xl shadow-xl border border-neutral-100 overflow-hidden flex flex-col max-h-[300px]">
          ...
          <div className="flex-1 overflow-y-auto p-1 custom-scrollbar">
            <div 
              ...
              className={`w-full text-left px-3 py-2.5 rounded-xl text-sm transition-colors cursor-pointer flex items-center justify-between ${value === "" ? 'bg-[#f3f0ff] text-[#7c3aed] font-semibold' : 'text-neutral-700 hover:bg-neutral-50'}`}
            >
              <span>Any Plan Tier</span>
              {value === "" && <div className="w-2 h-2 rounded-full bg-[#7c3aed]" />}
            </div>
            
            {options.map((opt) => (
              <div 
                key={(opt.ulid || opt._id)}
                onClick={() => {
                  onChange((opt.ulid || opt._id));
                  setIsOpen(false);
                  setSearch("");
                }}
                className={`w-full text-left px-3 py-2.5 rounded-xl text-sm transition-colors cursor-pointer flex items-center justify-between group ${value === (opt.ulid || opt._id) ? 'bg-[#f3f0ff] text-[#7c3aed] font-semibold' : 'text-neutral-900 hover:bg-neutral-50'}`}
              >
                <div className="flex flex-col">
                  <span className="truncate">{opt.name}</span>
                  <span className="text-[10px] text-neutral-500">{opt.category} - {opt.days} Days</span>
                </div>
                {value === (opt.ulid || opt._id) && (
