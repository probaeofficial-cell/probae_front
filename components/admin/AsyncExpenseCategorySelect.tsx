...
interface AsyncExpenseCategorySelectProps {
  value: string;
  onChange: (ulid: string) => void;
  selectedCategory?: ExpenseCategory | null;
}

export default function AsyncExpenseCategorySelect({ value, onChange, selectedCategory }: AsyncExpenseCategorySelectProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [options, setOptions] = useState<ExpenseCategory[]>([]);
  ...
  const fetchOptions = async (pageNum: number, searchTerm: string, isNewSearch: boolean) => {
    setIsLoading(true);
    try {
      const res = await endpoints.expenses.listCategories(true, pageNum, 10, searchTerm) as any;
  ...
  const currentOption = options.find(o => o.ulid === value) || selectedCategory;
  const displayLabel = value === "" ? "Select Category…" : currentOption ? currentOption.name : "Select Category…";

  return (
    <div className="relative w-full" ref={containerRef}>
      <div 
        className={`w-full h-[48px] bg-[#f8f5fb] border-neutral-200 rounded-xl px-4 flex items-center justify-between cursor-pointer transition-all border ${isOpen ? 'border-[#7c3aed] ring-2 ring-[#7c3aed]/20' : 'border-transparent'}`}
        onClick={() => setIsOpen(!isOpen)}
      >
        <span className={`text-sm font-medium ${currentOption || value === "" ? 'text-neutral-800' : 'text-neutral-500'} line-clamp-1`}>
          {displayLabel}
        </span>
          ...
          <div className="flex-1 overflow-y-auto p-1 scrollbar-thin">
            {options.map((opt) => (
              <div 
                ...
                className={`w-full text-left px-3 py-2.5 rounded-xl text-sm transition-colors cursor-pointer flex items-center justify-between group ${value === opt.ulid ? 'bg-[#f3f0ff] text-[#7c3aed] font-semibold' : 'text-neutral-700 hover:bg-neutral-50'}`}
              >
